...
public class TestThreeSpeakers {
  ...
  public void testThreeSpeakers() throws Exception {
    ...
    for (int second = 0;; second++) {
    	...
    	try { if (driver.findElement(By.xpath("//div[@id='addtocart']/div/div/div[2]/h3")).getText().matches("^cart summary[\\s\\S]*$")) break; } catch (Exception e) {}
    	Thread.sleep(1000);
    }
    
    assertEquals("1  item added to cart", driver.findElement(By.xpath("//div[@id='addtocart']/div/div/div/div/h2")).getText());
    assertEquals(nameOfSpeaker, driver.findElement(By.xpath("//div[2]/ul/li/div[2]/a")).getText());
    try {
      assertEquals(Integer.toString(numSpeakers), driver.findElement(By.id("cartUpdatedQty_cartItem0001")).getAttribute("value"));
    } catch (Error e) {
